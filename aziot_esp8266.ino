...
// Write temp
 ...
 // Serial.println(temperature);
  ...
  uint32_t l = lightMeter.readLightLevel();
  (void)az_span_u32toa(temp_span, l , &temp_span);  

    // Write soilmoisture
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(", \"soilmoisture\": "));
  uint32_t m = (uint32_t)analogRead(MOISTPIN);
  double percentMoist = (double)(1024L-m)/(double)1024L;
  (void)az_span_dtoa(temp_span, percentMoist , 2,  &temp_span);  
